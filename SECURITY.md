...
`machinectl` intentionally exposes high-privilege local capabilities to an authenticated MCP caller. Its core `shell` tool is terminal-equivalent and enabled by default; optional delegated-agent harness tools control local session content and execution when explicitly enabled. Pi is the first adapter. This is a deliberate trust model, not a sandbox boundary.
