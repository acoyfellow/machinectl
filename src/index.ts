...
// ─── auth model ──────────────────────────────────────────────────────────
//
// The Worker's /machinectl/connect is behind Cloudflare Access. The
// daemon authenticates with a Cf-Access-Authenticated-User-Email-equivalent
// header generated by `cloudflared access login` against the my-ax
// hostname. Specifically:
//
//   1. User runs `cloudflared access login https://my.ax.cloudflare.dev`
//      once, gets a long-lived JWT cached at ~/.cloudflared/.
//   2. The daemon calls `cloudflared access token --app=https://my.ax.cloudflare.dev`
//      at startup (and on reconnect) to extract that JWT.
...
// ─── config ──────────────────────────────────────────────────────────────
const URL_BASE = process.env.MACHINECTL_URL ?? "https://my.ax.cloudflare.dev";
